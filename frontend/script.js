        ...
        portfolioProjects = data.map(p => {
          ...
          return {
            ...
            tag: ({
              ...
              motion: 'Motion Graphics', video: 'Video Production', ui: 'UI Design', web: 'Web Design',
...
function renderFeaturedWork() {
  ...
  if (!grid) return;
  // Clear any existing card timers
  cardTimers.forEach(t => clearInterval(t));
  ...
  grid.innerHTML = display.map((p, i) => {
    const slides = buildCardSlides(p);
    return `<div class="work-card${i===0?' featured':''}${delayClass(i)} reveal" data-project="${p.id}" tabindex="0" role="button" aria-label="View ${p.title} project" style="position:relative;overflow:hidden">
      ${slides}<div class="work-card-border" aria-hidden="true"></div>
  ...
  grid.querySelectorAll('[data-project]').forEach(card => {
    ...
    // Start Netflix cycling for cards with multiple media
    const p = projectData[id];
    if (p) {
      const mediaList = p.media && p.media.length ? p.media : (p.img ? [{ url: p.img, media_type: 'image' }] : []);
      startCardCycle(card, mediaList);
    }
  });
}
