...
// ── HELPER: Replace all project_media for a project atomically ──────────────
// Called by POST and PUT when the client sends a full media_urls array.
// This is the single source of truth — the client sends the complete ordered
// list after every save, so we DELETE old records and INSERT fresh ones.
async function syncMediaUrls(projectId, mediaUrls) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    // Delete existing media for this project
    await client.query('DELETE FROM project_media WHERE project_id = $1', [projectId]);
    // Insert all media items in the new order
    let thumbSet = false;
    for (const m of mediaUrls) {
      const isThumb = !!m.is_thumbnail && m.media_type !== 'video';
      if (isThumb) thumbSet = true;
      await client.query(
        `INSERT INTO project_media
           (project_id, url, storage_key, media_type, mime_type, sort_order, is_thumbnail)
         VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [projectId, m.url, m.key||null, m.media_type||'image',
         m.mime_type||null, m.sort_order??0, isThumb]
      );
    }
    // Auto-assign thumbnail to first image if none set
    if (!thumbSet) {
      await client.query(
        `UPDATE project_media SET is_thumbnail = TRUE
         WHERE id = (
           SELECT id FROM project_media
           WHERE project_id = $1 AND media_type != 'video'
           ORDER BY sort_order ASC, created_at ASC LIMIT 1
         )`,
        [projectId]
      );
    }
    // Update project thumbnail_url and media_count from the new records
    await client.query(
      `UPDATE projects SET
         ...
         thumbnail_url = COALESCE(
           ...
           (SELECT url FROM project_media WHERE project_id = $1 AND media_type != 'video' ORDER BY sort_order ASC LIMIT 1),
           (SELECT url FROM project_media WHERE project_id = $1 ORDER BY sort_order ASC LIMIT 1),
           image_url
         )
       WHERE id = $1`,
      [projectId]
    ...
    console.error('[syncMediaUrls]', err.message);
    throw err;
  } finally {
    client.release();
  }
...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Sync media_urls into project_media table
    const mediaUrls = Array.isArray(req.body.media_urls) ? req.body.media_urls : [];
    if (mediaUrls.length) {
      await syncMediaUrls(newProject.id, mediaUrls);
    }
    const [withMedia] = await attachMedia([newProject]);
...
router.put('/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!rows[0]) return res.status(404).json({ error: 'Project not found.' });
    // Sync media_urls into project_media if provided
    const mediaUrls = Array.isArray(req.body.media_urls) ? req.body.media_urls : null;
    if (mediaUrls !== null) {
      await syncMediaUrls(Number(req.params.id), mediaUrls);
    }
    const [project] = await attachMedia([rows[0]]);
    res.json(project);
  } catch (err) {
